...
class ConfusionMatrix(object):
    ...
    def compute(self):
        h = self.mat.float()
        # 计算全局预测准确率(混淆矩阵的对角线为预测正确的个数)(就是准确率,accuracy)
        acc_global = torch.diag(h).sum() / h.sum()
        # 计算每个类别的准确率(应该就是召回率,recall)
        acc = torch.diag(h) / h.sum(1)
        # 计算每个类别的精确率(precision)
        precision = torch.diag(h) / h.sum(0)
        # 计算每个类别预测与真实目标的iou
        iu = torch.diag(h) / (h.sum(1) + h.sum(0) - torch.diag(h))
        return acc_global, acc, precision, iu
    ...
    def __str__(self):
        acc_global, acc, precision, iu = self.compute()
        return (
            'global correct: {:.2f}\n'
            'average row correct: {}\n'
            'precision: {}\n'
...
class DiceCoefficient(object):
    def __init__(self, num_classes: int = 2, ignore_index: int = -100):
        ...
        self.background_cumulative_dice = None
        self.num_classes = num_classes
        self.ignore_index = ignore_index
        self.count = 0

    def update(self, pred, target):
        if self.cumulative_dice is None or self.background_cumulative_dice is None:
            #self.cumulative_dice = torch.zeros(1, dtype=pred.dtype, device=pred.device)
            self.cumulative_dice = 0
            self.background_cumulative_dice = 0
        # compute the Dice score, ignoring background
        #on-hot编码默认channel维度为最后，所以要进行维度的变换
        ...
        #从1开始是忽略背景，channel=0是背景
        self.cumulative_dice += multiclass_dice_coeff(pred[:, 1:], dice_target[:, 1:], ignore_index=self.ignore_index)
        self.background_cumulative_dice += multiclass_dice_coeff(pred[:, 0:], dice_target[:, 0:], ignore_index=self.ignore_index)
        self.count += 1
    ...
    def value(self):
        if self.count == 0:
            ...
            return [self.background_cumulative_dice/self.count, self.cumulative_dice / self.count]
